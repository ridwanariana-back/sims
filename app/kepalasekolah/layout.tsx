    ...
    <div className="flex h-screen bg-gray-100 text-slate-900 overflow-hidden relative">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto pt-4 pb-8 px-4 space-y-2 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-800 [&::-webkit-scrollbar-thumb]:rounded-full">
          ...
          <Link href="/kepalasekolah/dataguru" className={getLinkStyle('/kepalasekolah/dataguru')} onClick={() => setIsOpen(false)}>
            Data Guru
          </Link>
          <Link href="/kepalasekolah/datamurid" className={getLinkStyle('/kepalasekolah/datamurid')} onClick={() => setIsOpen(false)}>
            Data Murid
          </Link>
          <Link href="/kepalasekolah/kedisiplinan" className={getLinkStyle('/kepalasekolah/kedisiplinan')} onClick={() => setIsOpen(false)}>
            Catatan Kedisiplinan
          </Link>
          <Link href="/kepalasekolah/rekap-nilai" className={getLinkStyle('/kepalasekolah/rekap-nilai')} onClick={() => setIsOpen(false)}>
            Rekap Nilai Murid
          </Link>
          <Link href="/kepalasekolah/monitoring-jadwal" className={getLinkStyle('/kepalasekolah/monitoring-jadwal')} onClick={() => setIsOpen(false)}>
            Jadwal Pelajaran
          </Link>
          <Link href="/kepalasekolah/kehadiran" className={getLinkStyle('/kepalasekolah/kehadiran')} onClick={() => setIsOpen(false)}>
            Kehadiran
          </Link>
          <Link href="/kepalasekolah/prestasi" className={getLinkStyle('/kepalasekolah/prestasi')} onClick={() => setIsOpen(false)}>
            Prestasi
          </Link>
          <Link href="/kepalasekolah/alumni" className={getLinkStyle('/kepalasekolah/alumni')} onClick={() => setIsOpen(false)}>
            Alumni
          </Link>
