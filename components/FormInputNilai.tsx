  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push("/guru/inputnilai/riwayat");
        router.refresh();
      }
    } catch (error) {
    ...
    <div className="space-y-6 text-left">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 pt-4">
          <Link href="/guru/inputnilai" className="flex-1 flex items-center justify-center gap-2 py-3.5 border-2 border-slate-100 rounded-xl font-bold hover:bg-slate-50 text-slate-600 transition-all text-sm">
